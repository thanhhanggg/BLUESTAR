...
export default function MainLayOut({ children }) {
    const pages = ["Ticket", "Seat", "Luggage","Food", "Payment"];
    const [isActive, setIsActive] = useState("Ticket");
    const navigate = useNavigate(); // Lấy đối tượng history từ React Router

    const handleNextClick = () => {
        const currentPageIndex = pages.indexOf(isActive);
        if (currentPageIndex < pages.length - 1) {
            const nextPage = pages[currentPageIndex + 1];
            setIsActive(nextPage);

            // Sử dụng navigate để chuyển trang
            navigate(`/${nextPage.toLowerCase().replace(" ", "")}`);
    ...
    const handleBackClick = () => {
        ...
        if (currentPageIndex > 0) {
            ...
            setIsActive(previousPage);
            navigate(`/${previousPage.toLowerCase().replace(" ", "")}`);
        }
    };
                                ...
                                <Paper className="fare-paper">
                                    ...
                                    <div className="button-wrapper">
                                        ...
                                        <Button variant="outlined" style={{
                                            borderColor: '#c5e1ef', width: '200px'
